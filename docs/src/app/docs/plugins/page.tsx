  ...
  return (
    <div className="space-y-8">
      ...
      <section>
        ...
        <p className="mt-2 text-slate-600">
          Add plugins in <code className="rounded bg-slate-100 px-1.5 py-0.5 font-mono text-sm">farm.config.ts</code> via
          the <code className="rounded bg-slate-100 px-1.5 py-0.5 font-mono text-sm">plugins</code> array. Plugins run in
          order; you can control order with <code className="rounded bg-slate-100 px-1.5 py-0.5 font-mono text-sm">enforce: "pre"</code> or{" "}
          <code className="rounded bg-slate-100 px-1.5 py-0.5 font-mono text-sm">"post"</code>.
        </p>
  ...
  return definePlugin({
    ...
    beforeRequest(req, _res, context) {
      ...
      <section>
        ...
        <p className="mt-2 text-slate-600">
          Import server plugins from <code className="rounded bg-slate-100 px-1.5 py-0.5 font-mono text-sm">@farmjs/core/plugin/server</code>.
        </p>
        ...
        <div className="mt-6 space-y-6">
          <div className="rounded-lg border border-slate-200 bg-slate-50/50 p-4">
            ...
            <p className="mt-1 text-sm text-slate-600">
              ...
              <code className="rounded bg-slate-100 px-1 py-0.5 font-mono text-xs">beforeRequest</code> and{" "}
              <code className="rounded bg-slate-100 px-1 py-0.5 font-mono text-xs">afterResponse</code> overrides.
            </p>
...
plugins: [createLoggerPlugin({})]`}
          ...
          <div className="rounded-lg border border-slate-200 bg-slate-50/50 p-4">
            ...
            <p className="mt-1 text-sm text-slate-600">
              Sets <code className="rounded bg-slate-100 px-1 py-0.5 font-mono text-xs">Content-Encoding</code> (gzip/br) in production based on{" "}
              <code className="rounded bg-slate-100 px-1 py-0.5 font-mono text-xs">Accept-Encoding</code>. Typically used in production builds.
            </p>
            <pre className="mt-3 overflow-x-auto rounded bg-slate-900 p-3 text-xs text-slate-100">
...
plugins: [createCompressionPlugin({})]`}
          ...
          <div className="rounded-lg border border-slate-200 bg-slate-50/50 p-4">
            ...
            <p className="mt-1 text-sm text-slate-600">
              Redirects by path pattern. Use with <code className="rounded bg-slate-100 px-1 py-0.5 font-mono text-xs">redirects()</code> in config.
            </p>
            <pre className="mt-3 overflow-x-auto rounded bg-slate-900 p-3 text-xs text-slate-100">
...
// Plugin is applied when you use redirects in farm.config`}
          ...
          <div className="rounded-lg border border-slate-200 bg-slate-50/50 p-4">
            ...
            <p className="mt-1 text-sm text-slate-600">
              Adds response headers by path. Use with <code className="rounded bg-slate-100 px-1 py-0.5 font-mono text-xs">headers()</code> in config.
            </p>
            <pre className="mt-3 overflow-x-auto rounded bg-slate-900 p-3 text-xs text-slate-100">
              {`async headers() {
  return [
    {
      ...
      headers: [
        ...
        { key: "X-Content-Type-Options", value: "nosniff" },
          ...
          <div className="rounded-lg border border-slate-200 bg-slate-50/50 p-4">
            ...
            <p className="mt-1 text-sm text-slate-600">
              Rewrites URL paths internally. Use with <code className="rounded bg-slate-100 px-1 py-0.5 font-mono text-xs">rewrites()</code> in config.
            </p>
          </div>
          ...
          <div className="rounded-lg border border-slate-200 bg-slate-50/50 p-4">
            ...
            <p className="mt-1 text-sm text-slate-600">
              Injects environment variables or env-based behavior into the request/response pipeline.
            </p>
          </div>
        </div>
      </section>
      ...
      <section>
        ...
        <p className="mt-2 text-slate-600">
          Plugins can set <code className="rounded bg-slate-100 px-1.5 py-0.5 font-mono text-sm">enforce: "pre"</code> to run
          before normal plugins, or <code className="rounded bg-slate-100 px-1.5 py-0.5 font-mono text-sm">enforce: "post"</code> to run after.
          Request hooks (<code className="rounded bg-slate-100 px-1 py-0.5 font-mono text-xs">beforeRequest</code>,{" "}
          <code className="rounded bg-slate-100 px-1 py-0.5 font-mono text-xs">afterResponse</code>) run in that order and can short-circuit the pipeline.
        </p>
      </section>
      ...
      <section className="rounded-lg border border-emerald-200 bg-emerald-50/50 p-6">
        ...
        <p className="mt-2 text-slate-600">
          You can define custom plugins with <code className="rounded bg-slate-100 px-1.5 py-0.5 font-mono text-sm">definePlugin</code> and
          hook into config, requests, responses, and build. See the guide below.
        </p>
        <Link
