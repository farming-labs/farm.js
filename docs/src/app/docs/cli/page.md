...
Farm detects the running app on common local ports, or you can pass `--port` / `--url` when the app is running somewhere specific. The command keeps forwarding traffic until you press Ctrl+C.
