...
export function generateServiceWorker(options: GenerateServiceWorkerOptions): string {
  ...
  const cacheScope = createHash("sha256")
    ...
    .digest("hex")
    .slice(0, 16);
  const precachePrefix = `farm-pwa-precache-${cacheScope}-`;

  return `/* Generated by @farm.js/pwa. */
const PRECACHE_PREFIX = ${JSON.stringify(precachePrefix)};
const PRECACHE = ${JSON.stringify(`${precachePrefix}${options.cacheId}`)};
const IMAGE_CACHE = ${JSON.stringify(`farm-pwa-images-${cacheScope}-v1`)};
