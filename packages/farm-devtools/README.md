...
A development-only workspace for Farm.js routes, APIs, integrations, runtime diagnostics, and
browser modules. Uses the Farm logo, bundled Geist fonts, light/dark themes, and syntax-highlighted
...
is the opt-in; there is no `enabled` option. Existing top-level DevTools configuration remains
supported without the plugin. Do not combine it with `devtools: false`.
