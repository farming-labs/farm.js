...
function fixQueryFiles() {
  ...
  if (clientMatch) {
    ...
    const rootParsersMatch = fs.readdirSync(distDir).find((file) => /^parsers-.*\.d\.mts$/.test(file));
    if (rootParsersMatch) {
      ...
      content = content.replace(/\/\/ Explicit parser type declarations for proper type inference[\s\S]*?declare function createParser[^;]+;\n\n/g, '');
    }
