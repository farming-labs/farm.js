...
import { defineEventHandler, getHeaders, readRawBody, sendWebResponse, setResponseStatus, getRequestURL } from 'h3';
import type { H3Event } from 'h3';
import { farmRegistry } from '../farm-registry';
import { createFarmLegacyRequest } from '@farm.js/core/internal/production-runtime';
...
export default defineEventHandler(async (event: H3Event) => {
  ...
  try {
    ...
    const request = await createFarmLegacyRequest(event, fullUrl, headers, readRawBody);
...
import { defineEventHandler, setResponseStatus, getRequestURL, setHeader, getHeaders, readRawBody, sendWebResponse, setCookie } from 'h3';
import type { H3Event } from 'h3';
import { farmRegistry } from '../farm-registry';
import React from 'react';
import { renderToString } from 'react-dom/server';
import { searchParamsToObject } from '@farm.js/core/internal/production-runtime';
import { createFarmLegacyRequest, renderFarmLegacyErrorHtml, renderFarmLegacyHtml } from '@farm.js/core/internal/production-runtime';
...
export default defineEventHandler(async (event: H3Event) => {
  try {
    ...
    if (apiRouteManager?.matchRoute(pathname)) {
      ...
      const request = await createFarmLegacyRequest(event, url, headers, readRawBody);
