...
describe("createFarmDocsHandler", () => {
  ...
  it("serves docs markdown files as HTML", async () => {
    ...
    expect(html).toContain(
      ".sidebar-folder-content > a[data-active]::after { left: calc(var(--fd-sidebar-guide-x) + var(--fd-sidebar-branch-gap) + 1px);",
    );
    expect(html).toContain(
    ...
    expect(html).toContain(
      ".sidebar-subgroup-content a[data-active]::after { left: calc(var(--fd-sidebar-sub-guide-x) + 1px);",
    );
    expect(html).toContain(
