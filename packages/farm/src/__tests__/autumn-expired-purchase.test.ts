...
describe("autumn /billing/status classifies expired one-time purchases", () => {
  ...
  it("keeps an older lifetime purchase active when a newer purchase has expired", async () => {
  ...
  it("keeps an older unexpired purchase active when a newer purchase has expired", async () => {
    const now = Date.now();
    const integration = buildIntegration("multi-expiry-user", [
      { planId: "pro", expiresAt: now + HOUR, startedAt: now - 2 * HOUR, quantity: 1 },
      { planId: "pro", expiresAt: now - HOUR, startedAt: now - HOUR, quantity: 1 },
    ]);

    expect(await (await callGet(integration, "/billing/status")).json()).toMatchObject({
      status: "active",
      planId: "pro",
      productId: "pro",
    });
  ...
  it("uses the newest expired purchase when every matching purchase has expired", async () => {
    const now = Date.now();
    const integration = buildIntegration("all-expired-user", [
      { planId: "pro", expiresAt: now - 2 * HOUR, startedAt: now - 3 * HOUR, quantity: 1 },
      { planId: "pro", expiresAt: now - HOUR, startedAt: now - 2 * HOUR, quantity: 1 },
    ]);
