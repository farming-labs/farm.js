...
describe("createFarmDocsHandler Setext TOC", () => {
  ...
  it("derives TOC slugs from the same MDX-stripped source as the renderer", async () => {
    const html = await renderPage(
      "mdx-runtime-lines",
      ['export const Foo = "x"', "---", "", '## export const Foo = "x"'].join("\n"),
    );

    // The export is removed before Marked runs, so the underline is a thematic
    // break and the ATX heading owns the first slug.
    expect(html).toContain("<hr");
    expect(html).toContain('id="export-const-foo-x"');
    expect(html).not.toContain('id="export-const-foo-x-2"');
    const toc = tocSection(html);
    expect(toc).toContain('href="#export-const-foo-x"');
    expect(toc).not.toContain('href="#export-const-foo-x-2"');
  });
});
