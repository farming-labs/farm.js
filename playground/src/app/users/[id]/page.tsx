...
export default function UserPage({ params, searchParams }: PageProps) {
  ...
  return (
    <div>
      ...
      <h1 style={{ color: "#1e293b", marginBottom: "0.5rem" }}>{userData.name}</h1>

      <p style={{ color: "#64748b", marginBottom: "2rem" }}>User ID: {userId}</p>
...
function StatCard({ title, value }: { title: string; value: string | number }) {
  return (
    ...
    >
      ...
      <div style={{ color: "#64748b", fontSize: "0.875rem" }}>{title}</div>
    </div>
  );
}
